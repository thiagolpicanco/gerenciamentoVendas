...
public abstract class Dao<T> {
	...
	if (this.classeEntidade == null) {
		...
		Field tipoArgumentoAtributoAtual = tipoSuperClasseGenerica.getClass().getDeclaredField(
			"actualTypeArguments");
	...
	if (entidade != null) {
		...
		if (valorAtributo != null) {
			...
			if (!((String) valorAtributo).trim().isEmpty()) {
			    consulta.append(" AND UPPER(entidade." + atributo.getName() + ") LIKE :"
				    + atributo.getName());
			    filtroMap
				    .put(atributo.getName(), "%" + valorAtributo.toString().trim().toUpperCase() + "%");
			}
